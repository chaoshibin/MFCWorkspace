...
// CHardwareInfoDlg 对话框




CHardwareInfoDlg::CHardwareInfoDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CHardwareInfoDlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(CHardwareInfoDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
END_MESSAGE_MAP()
...
BOOL CHardwareInfoDlg::OnInitDialog()
{
	...
	char* mac = getMac();
	CString strMac = mac;
	SetDlgItemText(IDC_EDIT_MAC,strMac);

	char* cpuid=getcid();
	CString  strCpuId = cpuid;
	SetDlgItemText(IDC_EDIT_CPUID,strCpuId);

	char* diskid=getdid();
	CString  strDiskId = diskid;
	SetDlgItemText(IDC_EDIT_DISKID,strDiskId);
